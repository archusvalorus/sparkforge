...
**v2.0.1 hotfix sprint IN PROGRESS** (`docs/v2.0.1-kickoff.md`): Arena 5 was un-unlockable in prod — the Faceted Lie's `onDeath` never set `arenasUnlocked = 5`, masked all through dev by a DEBUG force-unlock (now removed; debug builds run the real progression). Both units DONE + Brandon-approved (Unit 1 unlock/heal/card text; Unit 2 boss-spawn camera reveal, timings in `GameConfig.BossReveal`). **SUBMITTED to App Review Aug 9 2026** (tag `v2.0.1-build8`, ASC notes updated to the subtitle knock) — awaiting verdict; on approval flip this line + write the Notion chain-of-events. **v2.1 IN PROGRESS — ONE release: Arena 6 + the ability rework together (Brandon, Sep 15; date pushed back 1–2 weeks rather than split; Neutral unscheduled)** (`docs/v2.1-kickoff.md` → superseded in sequence by `docs/arena6-geometry-reconciliation.md`, Lyra's design lock at `docs/arena6-splitworks-design-lock.md`): ✅ Unit 0 arena unlock registry · ✅ geometry recon · ✅ **Geometry Unit 1A foundation + Splitworks shell** (`ArenaGeometry.swift`, return packet at `docs/v2.1-geometry-1a-return.md`) · card/ability rework DECIDED Sep 15 (`docs/v2.1-ability-decisions-handoff.md` → spec reconciled; build plan `docs/v2.1-abilities-kickoff.md`, open items `docs/v2.1-ability-closure-table.md`; Track 2 runs in its own thread) · ✅ **signature DRAW mechanics SHIPPED to main (Track 2 Unit 1, Brandon-approved Sep 2026; `docs/v2.1-signature-draw-return.md`)** · ✅ Unit 1B route graph (`81ea7ab`) · ✅ **Unit 2a travel/vision policies (Brandon device-gated Sep 8, `d678298`)** · ✅ **Unit 2b Spurhound (Brandon device-gated Sep 15, `7e50c98`)** · ✅ **Unit 2c Linekeeper (Brandon device-gated Sep 15, `4897e4c`)** · ✅ **Unit 2d Ramplate (Brandon device-gated Sep 15, `c6d958b`) — ROSTER COMPLETE** · ✅ **Unit 3 Marchwarden (Brandon device-gated Sep 15)** · SEQUENCING (Sep 15): ability rework (Track 2, own thread) wraps FIRST, then geometry resumes → Unit 4 presentation (registry, skin, bestiary, sound) → Unit 5 closeout → ASC. Card Atlas = standing visual aid (`tools/generate-card-atlas.py`). v2.0.5 shelf: Arena 5 bestiary ✅ built; Mote sprite art-blocked.
...
- Physics categories are a bitmask ladder; **next free bit is `0x1 << 9`** (`0x1 << 8` is `forgeCoin`, claimed in v1.8). `GameConfig.Physics` is the source of truth — check it before claiming, and update the App Portfolio Registry in Notion when you do.
