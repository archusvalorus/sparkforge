    ...
    private static let ladders: [Branch: [Node]] = [
        ...
        .ferocity: [
            ...
            Node(fork: 5, "Risk or Reliability",
                 a: ForkOption(name: "Berserker", effectText: "Below 50% HP: +5% damage") { $0.forgeBerserker = true },
            ...
            Node(fork: 10, "Swarms or Priority",
                 a: ForkOption(name: "Cleaver", effectText: "4+ enemies near: +8% damage") { $0.forgeCleaver = true },
                 b: ForkOption(name: "Headsman", effectText: "+10% damage to boss-class") { $0.forgeHeadsman = true }),
            Node(11, "Forged Edge", "+2 ATK") { $0.baseAttack += 2 },
            Node(12, "White Heat", "+1.5% damage") { $0.damageMultiplier += 0.015 },
            Node(13, "Rising Heat", "A kill grants +1% damage for 3s (stacks 5)") { $0.forgeRisingHeat = true },
            Node(14, "Opening Blow", "+10% damage to enemies above 90% HP") { $0.forgeOpeningBlow = true },
            Node(fork: 15, "Momentum or Patience",
                 a: ForkOption(name: "Bloodrush", effectText: "A kill grants +5% attack speed for 4s (stacks 3)") { $0.forgeBloodrush = true },
                 b: ForkOption(name: "Cold Fury", effectText: "Every 8s without a kill, +10% to the next enemy hit") { $0.forgeColdFury = true }),
            Node(16, "Hammered Point", "+2 ATK") { $0.baseAttack += 2 },
            Node(17, "Roaring Furnace", "+1.5% damage") { $0.damageMultiplier += 0.015 },
            Node(18, "Relentless Pressure", "Repeated hits on one foe: +1%/stack to +5%") { $0.forgeRelentless = true },
            Node(19, "Overkill", "On kill, excess damage bursts to nearby foes") { $0.forgeOverkill = true },
            Node(fork: 20, "Sustained or Decisive",
                 a: ForkOption(name: "Warpath", effectText: "After 6s of continuous damage, +10% damage") { $0.forgeWarpath = true },
                 b: ForkOption(name: "Killing Stroke", effectText: "Every 12s, the next direct attack deals +75%") { $0.forgeKillingStroke = true }),
        ],
        .cunning: [
            ...
            Node(fork: 10, "Collection or Escape",
                 ...
                 b: ForkOption(name: "Slipstream", effectText: "After a near-miss, +8% move speed for 2s") { $0.forgeSlipstream = true }),
            Node(11, "Trained Eye", "+1.5% crit chance") { $0.critChance += 0.015 },
            Node(12, "Swift Form", "+1.5% move speed") { $0.moveSpeedMultiplier += 0.015 },
            Node(13, "Opportunist", "+5% damage to impaired foes") { $0.forgeOpportunist = true },
            Node.behavioral(14, "Efficient Sweep", "Collecting XP grants +5% pickup for 2s (stacks 3)"),
            Node(fork: 15, "Consistency or Volatility",
                 a: ForkOption(name: "Calculated Strike", effectText: "Every 5th direct attack is a guaranteed crit") { $0.forgeCalculatedStrike = true },
                 b: ForkOption(name: "Lucky Break", effectText: "Crits have a 10% chance to deal +50% crit damage") { $0.forgeLuckyBreak = true }),
            ...
            Node(18, "Read the Room", "When an elite/boss enters, +8% move speed for 4s") { $0.forgeReadRoom = true },
            Node.behavioral(19, "Salvager", "Forge XP Coins are attracted from 10% farther"),   // no coin-magnet system yet
            Node(fork: 20, "Planned or Emergency",
                 a: ForkOption(name: "Foresight", effectText: "+1 reroll per run") { $0.forgeForesight = true },
                 b: ForkOption(name: "Second Look", effectText: "Once/run: passing a whole offer regenerates it free") { $0.forgeSecondLook = true }),
        ],
    ]
