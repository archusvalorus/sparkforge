...
{sections}
<footer class="page">Generated by <code>tools/generate-card-atlas.py</code> from <code>UpgradeManager.buildCardPool()</code>. Tier text is the live in-game copy. \U0001F451 = tree capstone. ★ = the tree's SIGNATURE (thesis card; the deterministic first pick — Guard's is INTERIM until its rework unit re-authors it). "AoE lens" = the capstones-need-AoE audit (⚠ = single-target falloff candidate). Gate lines show the provides/requires eligibility layer. Reference card <code>ids</code> when proposing changes.</footer>
</div>'''
